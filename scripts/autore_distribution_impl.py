...
import sys
import tempfile
from typing import Any, BinaryIO, Iterable, NamedTuple, Optional
...
RELEASE_MANIFEST_POLICY = ControlJsonPolicy(
    "release_manifest", 4 * 1024 * 1024, 64, 262_144, 100_000, 1024 * 1024
)
MANAGED_MARKER_POLICY = ControlJsonPolicy(
    "managed_marker", 256 * 1024, 32, 4_096, 1_024, 64 * 1024
)
...
def load_json_object(
    ...
    label: str,
    *,
    policy: ControlJsonPolicy = RELEASE_MANIFEST_POLICY,
) -> dict[str, Any]:
    try:
        with path.open("rb") as handle:
...
def validate_manifest(
    ...
    if distribution_scope == "repository":
        if package_target is not None:
            raise DistributionError(
                "repository distribution must not set package_target"
            )
...
def load_managed_marker(path: pathlib.Path, kind: str) -> Optional[dict[str, Any]]:
    ...
    marker = load_json_object(path, "managed marker", policy=MANAGED_MARKER_POLICY)
    if (
